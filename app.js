...
import { TakanoriAudioEngine } from './audioEngine.js';
...
const ICON_MOON = `<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12.79A9 9 0 0 1 11.21 3 7 7 0 0 0 21 12.79z"></path></svg>`;
...
const ICON_REPEAT = `<svg class="icon" viewBox="0 0 100 100" width="24" height="24" fill="none" stroke="currentColor" stroke-width="8" stroke-linecap="round" stroke-linejoin="round" style="width:24px; height:24px; display:block;"><path d="M 50 20 A 30 30 0 1 0 80 50"></path><polyline points="67,48 80,32 93,48"></polyline></svg>`;
class TakanoriVocabApp {
    ...
    constructor() {
        ...
        this.audioEngine = new TakanoriAudioEngine();
    }
    async start() {
    ...
    applyFilter(isInitialLoad = false, skipRender = false, explicitTargetId) {
        this.isFilterDirty = false;
        let targetWordId = null;
        if (explicitTargetId !== undefined) {
            targetWordId = explicitTargetId;
        }
    ...
    renderEmptyState() {
        ...
        if (this.elBtnRepeatAudio) {
            this.elBtnRepeatAudio.style.display = 'none';
        }
        this.stopAudio();
    ...
    bindDomElements() {
        ...
        let repeatBtn = document.getElementById('btn-repeat-audio');
        if (!repeatBtn) {
            repeatBtn = document.createElement('button');
            repeatBtn.id = 'btn-repeat-audio';
            repeatBtn.className = 'btn-repeat-audio';
            repeatBtn.setAttribute('aria-label', '音声リピート');
        }
        ...
        const cardContainer = this.elDynamic?.parentElement || document.querySelector('.card-area') || document.body;
        if (cardContainer && cardContainer !== document.body) {
            if (window.getComputedStyle(cardContainer).position === 'static') {
                cardContainer.style.position = 'relative';
            }
        }
        if (repeatBtn.parentElement !== cardContainer) {
            cardContainer.appendChild(repeatBtn);
        }
        this.elBtnRepeatAudio = repeatBtn;
    ...
    attachEventListeners() {
        ...
        this.setupAudioButtonEvents();
        if (this.elBtnRepeatAudio) {
            this.elBtnRepeatAudio.addEventListener('click', (e) => {
                e.stopPropagation();
                this.unlockAudio();
    ...
    renderCurrentCard(skipAudio = false) {
        this.stopAudio();
        if (this.displayWords.length === 0)
            return;
        ...
        if (this.elBtnRepeatAudio) {
            if (this.isAudioLocked) {
                this.elBtnRepeatAudio.style.display = 'inline-flex';
            }
            else {
                this.elBtnRepeatAudio.style.display = 'none';
            }
        }
        if (this.isAudioLocked && !skipAudio) {
            this.playCurrentSmartAudio();
        }
    }
    toggleFlip() {
    ...
    nextWord() {
        ...
        if (this.isFilterDirty) {
            const currentWord = this.displayWords[this.currentIndex];
            const nextWordInCurrentList = this.displayWords[this.currentIndex + 1];
            const targetId = nextWordInCurrentList ? nextWordInCurrentList.id : currentWord.id;
            this.applyFilter(false, true, targetId);
            if (this.displayWords.length === 0) {
                this.renderEmptyState();
                return;
            }
    ...
    prevWord() {
        ...
        if (this.isFilterDirty) {
            const currentWord = this.displayWords[this.currentIndex];
            const prevWordInCurrentList = this.displayWords[this.currentIndex - 1];
            const targetId = prevWordInCurrentList ? prevWordInCurrentList.id : currentWord.id;
            this.applyFilter(false, true, targetId);
            if (this.displayWords.length === 0) {
                this.renderEmptyState();
                return;

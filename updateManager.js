 ...
 *    - build-deploy.js により 1.0.20260908-015323 プレースホルダーへタイムスタンプが自動注入され、
...
export const APP_VERSION: string = '1.0.20260908-015323';

const TIMEOUT_MS: number = 2000;         // サーバー通信の厳格タイムアウト（2秒）
const GRACE_PERIOD_MS: number = 1200;    // 更新前トースト表示 ＆ キャッシュ破棄の猶予時間（1.2秒）
const SESSION_ATTEMPT_KEY: string = 'kotutan_update_attempted_ver';
const SESSION_COMPLETED_KEY: string = 'kotutan_update_just_completed';
 ...
 */
function showToast(message: string, isWarning: boolean = false): HTMLElement {
 ...
 */
function lockUserInteraction(): void {
...
export async function checkAndApplyUpdates(): Promise<void> {
  ...
  try {
    ...
    const remoteData = (await response.json()) as VersionData;
    ...
    if (APP_VERSION !== '1.0.20260908-015323' && remoteVersion !== APP_VERSION) {
